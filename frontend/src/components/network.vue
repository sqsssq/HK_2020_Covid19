 ...
 * @LastEditTime: 2023-05-25 13:18:20
   ...
   <template>
    <div class="frameworkTitle">
        <div class="title">Transmission View</div>
        <!-- <div class="slider-demo-block" style="width: 300px; float: right; margin-top: 7px; margin-right: 30px;">
                ...
                <!-- <div style="float: right; margin-top: 7px; margin-right: 10px; font-size: 18px;">
                    <svg height="30px" width="500px">
                        ...
                        <text x="5" y="21">Society Level:</text>
...
import distinctConnect from '../assets/distinct_directed_graph.csv';
    ...
    methods: {
        BeLarge() {
            ...
            const zoom = d3.zoom()
                .extent([
                    [0, 0],
                    [this.networkWidth, this.networkWidth]
                ])
                .scaleExtent([1, 20])
                .on("zoom", zoomed);
        ...
        drawDistinct() {
            console.log(distinctConnect, distinctType);
            let tmp_nodes = [];
            
            for (let i in distinctType) {
                tmp_nodes.push({
                    id: distinctType[i]['en_name'],
                    sum: parseInt(distinctType[i]['sum']),
                    type: parseInt(distinctType[i]['Color_type']),
                    t_color: this.class_color[parseInt(distinctType[i]['Color_type']) - 1]
                });
            }
            // console.log(nodes);
            let tmp_links = [];
            for (let i in distinctConnect) {
                if (parseInt(distinctConnect[i]['Num']) > 0)
                tmp_links.push({
                    source: distinctConnect[i]['Original_dist'],
                    target: distinctConnect[i]['Destination_dist'],
                    sum: parseInt(distinctConnect[i]['Num'])
                })
            }
            let link_extent = d3.extent(tmp_links, d => d.sum);
            let node_extent = d3.extent(tmp_nodes, d => d.sum);
            // console.log(node_extent, link_extent);
            let link_scale = d3.scaleLinear(link_extent, [0, 1]);
            let node_scale = d3.scaleLinear(node_extent, [5, 20]);
            // console.log(tmp_links);

            const links = tmp_links.map(d => Object.create(d));
            const nodes = tmp_nodes.map(d => Object.create(d));
            ...
            const simulation = d3.forceSimulation(nodes)
                ...
                .force("x", d3.forceX())
                .force("y", d3.forceY());
            let x_extent = d3.extent(nodes, d => d.x);
            let y_extent = d3.extent(nodes, d => d.y);
            let xScale = d3.scaleLinear(x_extent, [100, this.networkWidth - 100]);
            let yScale = d3.scaleLinear(y_extent, [100, this.networkHeight - 100]);

            for (let i in nodes) {
                nodes[i].nx = xScale(nodes[i].x);
                nodes[i].ny = yScale(nodes[i].y);
                nodes[i].nr = node_scale(nodes[i].sum);
                // if (nodes[i].relate == 0)
                //     nodes[i].nr = 4;
                // console.log(nodes[i].id);
                // if (nodes[i].id == 6774)
            ...
            for (let i in links) {
                // if (links[i]['target'] != 'Yau Tsim Mong')
                //     continue;
                links[i]['source']['nx'] = xScale(links[i]['source']['x']);
                links[i]['source']['ny'] = yScale(links[i]['source']['y']);
                links[i]['target']['nx'] = xScale(links[i]['target']['x']);
